...
def main() -> int:
    ...
    if not args.dry_run:
        ...
        if args.rebuild_index:
            ...
            cmd = [sys.executable, str(PROJECT_ROOT / "scripts" / "builders" / "build-index.py")]
            ...
            if args.rebuild_reports:
                ...
                subprocess.run([sys.executable, str(PROJECT_ROOT / "scripts" / "builders" / "build-progress.py")], check=True)
                print("[INFO] Rebuilding missing-report.json…")
                subprocess.run([sys.executable, str(PROJECT_ROOT / "scripts" / "builders" / "build-missing-report.py")], check=True)
